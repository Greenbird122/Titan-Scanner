...
> **Status:** Phases 0–4 DONE (built + mutation-proven) — **all four surface tracks shipped**. Only live in-scope validation per track remains.
> **Baseline:** `bb7d0ba4` (198 tests) → **303 tests passing, 3 skipped** (all phases added 105 tests, all mutation-checked).
> **New modules:** Track B — `bola/`, `massassignment/`, `jwt/`, `sessionfix/`, `core/sessions.py`, `verify/flows.py`, `verify/identity_oracles.py`. Track A — `clientside/` package. Track C — `llm/` package (`payloads.py`, `channel.py`, `detector.py`), `verify/llm_oracles.py`. Track D — `cloud/storage.py` probe, `verify/chain_analyzer.py` (flow-typed ChainAnalyzer; old shallow `verify/chain.py` deleted).
...
That model is rigorous (198 mutation-tested checks) but structurally blind to four things:
...
**Reviewer fixes applied during build:** OOB trials made independent (fresh URL per trial); model's actual compliant reply now stored as the finding body/evidence; dead `JUDGES` registry + unused params removed; channel fallback no longer short-circuits on empty 200s; system-leak judge gained a negative-directive gate (a model *describing* its guidelines no longer verifies as a leak); agency judge suppresses example-showing responses. Note: each probe spends the target's paid LLM inference — `llm.trials` is the quota lever.
...
```
Phase 0:  Foundation — Finding.flows typing + evidence taxonomy + lab fixtures   ✅ DONE (280 tests)
...
The project's non-negotiable is **evidence-based verification + mutation testing** (198 tests, every fix proven caught if reverted). Each track inherits that:
...
1. All four tracks shipped as engine capabilities, not bolted-on modules — each with its own runner seam in the engine (`_run_browser_modules`, `_run_identity_modules`, `_run_llm_channel`, `_run_storage_probe` + the scan-end chain pass).
...
4. Suite grows from 198 → ~260+ tests, all mutation-checked, all green. **(198 → 303)**
