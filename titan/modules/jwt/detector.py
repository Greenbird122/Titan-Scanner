...
class JWTDetector:
    ...
    def _forge_none_token(self, alg_value: str = "none") -> str:
        ...
        payload = self._b64(json.dumps({"sub": "titan_probe", "role": "admin", "admin": True, "iat": 1}).encode())
    ...
    def _sign_token_with_elevated_claims(self, token: str, secret: str) -> str:
        ...
        parts = token.split(".")
        header_b64 = parts[0] if len(parts) > 0 else self._b64(json.dumps({"alg": "HS256", "typ": "JWT"}).encode())
        try:
    ...
    async def _fetch_public_key(self, context, target: str, url: str) -> str | None:
        ...
        for jwks_path in (
            ...
            try:
                resp = await context.request.get(f"{base}{jwks_path}", headers={"Referer": target}, timeout=2000)
                body = await resp.text()
    ...
    def _finding(
        self, target, url, method, param, payload, body_snip, severity, confidence, resp, body, diffs
    ) -> Finding:
        return Finding(
            target=target,
