...
class CampaignPhasesMixin:
    ...
    async def _run_hostile_pass(self, target, result):
        ...
        candidates = [target] + [u for u in self.visited if self._is_in_scope(u) and not self._is_spa_shell(u)]
        ...
        async with aiohttp.ClientSession() as _hostile_session:
            payload = await run_pass(
                ...
                consented=consented,
                prior_observed=self._prior_observed(target),
            )
    ...
    def _detect_bypass(self, original_finding, resp) -> bool:
        ...
        try:
            ...
            if original_finding.attack_type.value == "SQLi" and any(
                m in body for m in ["sql", "syntax", "mysql", "sqlite", "postgres", "ORA-"]
            ):
                return True

            if original_finding.attack_type.value == "XSS" and any(
                m in body for m in ["<script", "alert(", "onerror", "onload"]
            ):
                return True
    ...
    async def _run_exploit_modules(self, target, result):
        ...
        for f in usable_findings(verified, target)[:max_per_type]:
            store = await guarded(
                ...
                stage_and_register(
                    f, target, listener, consent_dir=consent_dir, output_dir=output_dir, key_path=key_path
                ),
            )
    ...
    async def _run_fleet_scan(self, target, result):
        ...
        fleet_result = await coordinator.scan_all(
            targets=targets, agent_types=agent_types, budget=budget, context=context
        )
        ...
        for merged in fleet_result.merged_findings:
            exists = any(
                f.type == merged.type and f.url == merged.url and f.param == merged.param for f in result.findings
            )
            ...
            if not exists:
                try:
                    ...
                    finding = Finding(
                        type=AttackType(merged.type)
                        if merged.type in [e.value for e in AttackType]
                        else AttackType.OTHER,
