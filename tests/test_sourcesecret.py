...
def test_firebase_config_and_key_fire():
    ...
    # The label is bound to a name so the allowlist pragma stays attached to the
    # line carrying the literal. Inline on a wrapped comprehension it lands on
    # the closing bracket and the secrets gate flags the line the detector
    # actually points at.
    secret_label = "Firebase client config exposed"  # pragma: allowlist secret
    fb = [f for f in findings if f.metadata["secret_type"] == secret_label]
