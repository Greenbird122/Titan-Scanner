...
class TestChainAnalyzer:
    def test_ssrf_metadata_plus_hardcoded_key_forms_cloud_chain(self):
        ssrf = _mkf(
            AttackType.SSRF,
            url="http://t/proxy",
            flows=["url_fetch", "creds"],
            payload="http://169.254.169.254/latest/meta-data/",
        )
        key = _mkf(
            AttackType.CRYPTO_WEAKNESS, url="http://t/config", flows=["creds"], payload="hardcoded AWS key AKIA..."
        )
    ...
    def test_public_storage_finding_chains_with_ssrf(self):
        bucket = _mkf(AttackType.PUBLIC_STORAGE, url="http://bucket.s3.amazonaws.com/", flows=["data_leak"])
...
class TestStorageEngineWiring:
    async def test_storage_probe_runs_through_engine(self):
        ...
        result.findings.append(_mkf(AttackType.INFO_LEAK, body="https://engine-bucket.s3.amazonaws.com/x"))
    ...
    async def test_disabled_storage_skips(self):
        ...
        cfg = {"governance": {"enabled": False}, "ai": {"enabled": False}, "cloud": {"storage": {"enabled": False}}}
        engine = TitanEngine(cfg)
        called = []
        ...
        result = ScanResult(target="http://t", started_at=0)
        result.findings.append(_mkf(AttackType.INFO_LEAK, body="https://secret-bucket.s3.amazonaws.com/x"))
        await engine._run_storage_probe("http://t", result)
...
class TestReportChains:
    def test_report_contains_attack_chains_section(self):
        ...
        ssrf = _mkf(
            AttackType.SSRF, url="http://t/proxy", flows=["url_fetch", "creds"], payload="http://169.254.169.254/"
        )
        key = _mkf(AttackType.CRYPTO_WEAKNESS, url="http://t/config", flows=["creds"])
        chains = ChainAnalyzer().detect([ssrf, key])
        result = ScanResult(target="http://t", started_at=0)
