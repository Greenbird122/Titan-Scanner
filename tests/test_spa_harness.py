...
def test_select_runtime_apis_dedupes_and_scopes():
    ...
    out = select_runtime_apis(captured, ws_urls=ws, base_url="http://juice.local", scope_host="juice.local")
...
def test_select_runtime_apis_resolves_relative_and_subdomains():
    out = select_runtime_apis(["/rest/products"], base_url="http://juice.local", scope_host="juice.local")
    assert out == ["http://juice.local/rest/products"]
    # a subdomain of the scope host is in scope
    out2 = select_runtime_apis(["http://api.juice.local/v1/x"], scope_host="juice.local")
