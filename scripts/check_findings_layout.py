...
ROOT_ALLOWLIST = {
    ...
    "config.omega.yaml",
    "config.mkulima.yaml",
    "config.dt-hostile.yaml",
    "config.gv-hostile.yaml",
    "README.md",
    "REPRO.md",
    "SECURITY.md",
    "LICENSE",
...
def main():
    ...
    for f in sorted(os.listdir(ROOT)):
        ...
        if os.path.isfile(full) and is_stray(f, tokens):
            problems.append(f"STRAY  repo-root/{f}  -> belongs in findings/<slug>/ (e.g. mv {f} findings/<slug>/)")
    ...
    if os.path.isdir(FINDINGS):
        for f in sorted(os.listdir(FINDINGS)):
            ...
            if os.path.isfile(full) and is_stray(f, tokens):
                problems.append(
                    f"STRAY  findings/{f}  -> belongs in findings/<slug>/ (e.g. mv findings/{f} findings/<slug>/)"
                )
        ...
        for d in sorted(os.listdir(FINDINGS)):
            ...
            if os.path.isdir(full) and d not in slugs and d not in CONTAINER_DIRS:
                problems.append(
                    f"UNKNOWN findings/{d}/  not in consent/sites.json roster — verify the slug or register it"
                )
